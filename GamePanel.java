...
public class GamePanel extends JPanel implements ActionListener {
    ...
    public void draw(Graphics g) {
        if(running) {
            ...
            g.setFont(new Font("UBUNTU", Font.BOLD, 100));
    ...
    public void checkCollisions() {
        //checking if head snake collides with body
        for(int i = bodyParts; i > 0; i--) {
            if((x[0] == x[i] && (y[0] == y[i]))) {
                running = false;
            }
        }
        //checking if snake head touch left boreder;
        if(x[0] < 0) {
            running = false;
        }

        //if snake head touch right border
        if(x[0] > SCREEN_WIDTH) {
            running = false;
        }
